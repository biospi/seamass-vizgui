...
next_stream(double _mz_min, double _mz_max, double _rt_min, double _rt_max) //, double _max_counts)
{
	...
	*/
	...
	read_time.resume(); //read_start = omp_get_wtime();
...
next_chunk(const vector<Coef>& cs)
{
	// time chunk
	chunk_count += cs.size();
	read_time.stop(); //read_time += omp_get_wtime() - read_start;
	boost::chrono::duration<double> read_seconds = boost::chrono::nanoseconds(read_time.elapsed().user);
	stream_ofs << chunk_count << "," << read_seconds.count() << ","; //stream_ofs << chunk_count << "," << read_time << ",";
	...
	// time display
	viz_time.resume(); //viz_start = omp_get_wtime();
	...
	if (processed < cs.size())
	{
		...
		{
			vector< Triplets<fp> > listCy(nly); //vector< std::vector< Triplet<fp> > > listCy(nly);
			for (size_t i = 0; i < cs.size(); i++)
			if (cs[i].lx <= lx1 && cs[i].ly <= ly1)
			{
				...
				{
					listCy[cs[i].ly-ly0].push_back(_i, _j, _v); //(Triplet<double>(_i,_j,_v));
				}
			}
			...
			for (size_t ly = 0; ly < matCy.size(); ly++)
			{
				...
				listCy[ly].init(matCy[ly]); //matCy[ly].setFromTriplets(listCy[ly].begin(), listCy[ly].end());
		...
		for (size_t ly = 0; ly < matCy.size()-1; ly++)
		{
			...
			Triplets<fp> listAy; //vector< Triplet<fp> > listAy;
			...
			for (int i = 0; i < 5; i++)
			{
				...
				if (_i >= 0 && _i < matAy.rows())
				{
					...
					listAy.push_back(_i,_j,_v);  //(Triplet<double>(_i,_j,_v));
				}
			}
			listAy.init(matAy);	
	...
	viz_time.stop();  //viz_time += omp_get_wtime() - viz_start;
    boost::chrono::duration<double> viz_seconds = boost::chrono::nanoseconds(viz_time.elapsed().user);
	stream_ofs << viz_seconds.count() << endl;  //stream_ofs << viz_time << endl;
...
void Reconstructer :: fl_next_chunk(const vector<Coef>& cs, myOverlay* m_GUI, int ovwFlag)
{
	// ==== next_chunk(cs) ====
	...
	// if any left, use the b-spline recursive subdivision algorithm to efficiently display the remaining big basis functions
	if (processed < cs.size() && stopStream == 0)
	{
		// determine x bounds for each lx
		vector<int> x0(nlx);
		vector<int> x1(nlx);
		...
		for (size_t ly = 0; ly < matCy.size()-1; ly++)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				if (_i >= 0 && _i < matAy.rows())
				{
					int _j = j;
					fp _v = bspline::cbsf[i];
					listAy.push_back(_i,_j,_v);  //listAy.push_back(Triplet<double>(_i,_j,_v));
				}
			}
			listAy.init(matAy); 
			//matAy.setFromTriplets(listAy.begin(), listAy.end());	
	...
	fp frmMax = 0.0;
	for (int k = 0; k < img.outerSize(); ++k)
	for ( Matrix<fp,Dynamic,Dynamic>::InnerIterator it(img,k); it; ++it)
	{
		frmMax = frmMax > it.value() ? frmMax : it.value();
	}

	fp min_intensity = log(2.0*sqrt(3.0/8.0));
	fp max_intensity = log(2.0*sqrt(frmMax+3.0/8.0));
	...
	for (int y = 0; y < img.rows(); y++)
		for (int x = 0; x < img.cols(); x++)
		{
			...
			COLOUR c = GetColour(log(2.0*sqrt(img(y,x)+3.0/8.0)), min_intensity, max_intensity);
			long index = y * img.cols() * img_d + x * img_d;
			
			EnterCriticalSection(&g_cs); 
			m_GUI->RGB_data[index] = c.r*255.9999;
			m_GUI->RGB_data[index+1] = c.g*255.9999;
			m_GUI->RGB_data[index+2] = c.b*255.9999;
	...
	EnterCriticalSection(&g_cs); 
	m_GUI->imgData = img;
	m_GUI->maxCounts = max_counts;
	m_GUI->frmMaxCounts = frmMax;
	...
	EnterCriticalSection(&g_cs);
	if (m_GUI->omics_img_disp != NULL)
	{
		delete m_GUI->omics_img_disp;
		m_GUI->omics_img_disp = new Fl_Image(m_GUI->w(),m_GUI->h(),3);
	}
	m_GUI->omics_img_disp = m_GUI->omics_RGB_img->copy(m_GUI->box_main->w(),m_GUI->box_main->h());
	m_GUI->box_main->image(m_GUI->omics_img_disp);
	...
	Fl::lock();
	m_GUI->box_main->redraw_label();
	m_GUI->box_spectro->redraw();
	m_GUI->box_chroma->redraw();
	m_GUI->txt_cMap_max->value(s);
	
	Fl::unlock();

	
	// initial the thrumnail box
	if (ovwFlag == 0)
	{
		EnterCriticalSection(&g_cs);
		m_GUI->omics_nail_img = m_GUI->omics_RGB_img->copy(m_GUI->box_nav->w(),m_GUI->box_nav->h());
		m_GUI->box_nav->image(m_GUI->omics_nail_img);
		LeaveCriticalSection(&g_cs);

		Fl::lock();
		m_GUI->box_nav->redraw_label();
		Fl::unlock();
	}
}


void Reconstructer :: fl_close_stream_ofs()
{
	//stream_ofs.close();
}