...
class flQueryStrategy : public SpatialIndex::IQueryStrategy
{
private:
	const IShape& query;
	Reconstructer* recon;
...
public:
	flQueryStrategy(const IShape& _query,
		            Reconstructer* _recon,
					myOverlay* _m_GUI,
					int _chunk_size,
					int _overDisped) : 
	...
	void getNextEntry(const IEntry& entry, id_type& nextEntry, bool& hasNext)
	{
		...
		while ( stopStream == 0 && (node_q.empty() && !data_q.empty() ||
			   !data_q.empty() && data_q.top().r->getLow(2) <= node_q.top().r->getLow(2)))
		{
			//cout << ">>> stopStream = " << stopStream << endl;
			MyPair d = data_q.top();
			data_q.pop();
	...
	void visitData(const MyPair& pair, bool hasNext)
	{
		...
		if (index == chunk.size())		
		{
			...
			// reconstruct
			recon->fl_next_chunk(cs, m_GUI, ovwFlag);
...
SMVStreamer(const string& filename)
{
	...
	time.start(); //double start = omp_get_wtime();
	...
	//file = StorageManager::createNewRandomEvictionsBuffer(*diskfile, 1000, false); // use different cache size
	...
	cout << "Duration: " << seconds.count() << " seconds" << endl; //cout << "Duration: " << omp_get_wtime() - start << " seconds" << endl;
}
...
void
SMVStreamer::
stream_cs_gui(double _mz_min, double _mz_max, double _rt_min, double _rt_max,
          size_t chunk_size, Reconstructer* recon, myOverlay* m_GUI, int oDisped)
{
	_mz_min = mz_min > _mz_min ? mz_min : _mz_min;
	_mz_max = mz_max < _mz_max ? mz_max : _mz_max;
	_rt_min = rt_min > _rt_min ? rt_min : _rt_min;
	...
	flQueryStrategy qs(r, recon, m_GUI, chunk_size, oDisped);
	tree->queryStrategy(qs);

	// close the 'stream.csv' file
	//recon->stream_ofs.close();
